...
_RT_MUSIC_CATS = {6, 48}                 # «Музыка», «Аудио» — точно музыка
_RT_BAD_CATS = {2, 4, 7, 8, 11, 16, 17, 35, 42, 43, 45, 52, 55, 68, 71, 78}
# ↑ авто-мото, фильмы, мультфильмы, новости, путешествия, спорт, обучение, хобби,
#   детям, телепередачи, технологии, наука, лайфхаки, техника, здоровье,
#   обзоры/распаковки — всё это НЕ музыка.
...
def _rutube_search(query, n=30):
    ...
    for d in (data.get("results") or []):
        ...
        low = title.lower()
        if cat in _RT_BAD_CATS:
            continue                              # обзоры/техника/авто/наука/… — мимо
        if cat in _RT_MUSIC_CATS:
            if dur and dur > 1800:                # музыка, но часовой микс — мимо
                continue
        else:
            # неоднозначная категория («Разное», «Развлечения» и т.п.) — берём
            # только короткое (песня 20с..10мин) и без «обзорных» слов в названии.
            if dur and (dur < 20 or dur > 600):
                continue
